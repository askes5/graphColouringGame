...
\end{theorem}
%is this proof correct? Seems too easy
\begin{proof}
...
%     if $a \in X_i, a\notin X_j, b \in X_j$, and $i < j$, then  $a < b$.
     
    Let $v$ be any vertex in $V$, then by the definition of path width $d^+(v) \leq w$ and for all $i \in [1,n]$, $ \left| X_i \right| \leq w + 1 $. 
    
    %$v$ is in some $X_i$ where $N^+(v) \subset X_i$ and thus $N^+(v)$ is a clique. 
    
%    We now find matchings $M$ and $N$ that satisfy definition \ref{defnRank}.
    
%    Suppose $M=\{zs(z): z \in A\}$ is a matching from $A\subset N^-[v]$ to
%    $\{s(z) \in V^+(v) : z \in A\}$. Since $N^+(z)$ is a clique and $v \in N^+(z)$, $s(z)$ is adjacent to $v$, thus $\{s(z) : z \in A)\} \subset N^+(v)$. Therefore we have a matching such that $\left| A \right| = \{s(z) : z \in A)\} \leq w-2$.
    ...
    \begin{proof}
%        Let $u$ be a vertex in $P$ and $\mathcal{X} = \{X_i:v \in X_i\}$, that is $\mathcal{X}$ is the set of partitions that contain $u$. If $\left| \mathcal{X} \right|$ then $N(u) $ is a subset of the only element of $\mathcal{X}$ and we are done. Thus assume $\left| \mathcal{X} \right| \geq 2$ then by the definition of our linear order $L$, for each $X_i$ in $\mathcal{X}$ either $u > X_i\setminus{u}$ or $u < X_i\setminus{u}$.        
