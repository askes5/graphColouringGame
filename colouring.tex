...
\end{theorem}
%is this proof correct? Seems to easy
\begin{proof}
...
%    $\{s(z) \in V^+(v) : z \in A\}$. Since $N^+(z)$ is a clique and $v \in N^+(z)$, $s(z)$ is adjacent to $v$, thus $\{s(z) : z \in A)\} \subset N^+(v)$. Therefore we have a matching such that $\left| A \right| = \{s(z) : z \in A)\} \leq w-2$.
    ...
    By definition of path decomposition $v$ is in some $X_i$ such that $N^-(v) \subseteq X_i$. Let $Z\subset N^-[v]$, then $\left| Z\right| \leq \left| N^-(v)\right| \leq w+1$. Therefore by definition \ref{defnRank} $m(u,L,P) \leq w+1$ and $r(P) \leq r(L,P) \leq r(u,L,P) \leq 2w+2$. 
