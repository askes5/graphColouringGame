...
%TODO trim waffle
\begin{definition}[Kierstead \cite{KIERSTEAD2000}]
    Let $L$ be a linear order, $G=(V,E)$ be a graph, $u$ a vertex in $V$.
    
    Suppose $A,B$ are disjoint subsets of $V$. We say that a matching $M$ is a matching from $A$ to $B$ if $M$ saturates $A$ and $B\setminus A$ contains a cover of M. For $u \in V(G)$ the matching number $m(u, L, G)$ of $u$ with respect to $L$ in $G$ is defined to be the size of the largest set $Z \subset N^-[u]$ such that there exists a partition $[X, Y]$ of $Z$ and there exist matchings $M$ from
...
\begin{theorem}[Kierstead 1 \cite{KIERSTEAD2000}] \label{KIERSTEAD1}
    For any graph $G=(V, E$) and linear ordering $L$ on $V$, if Alice uses the strategy $S(L, G)$ to play the ordering game on $G$, then the score will be at most $1+r(L, G)$. In particular, col\textsubscript{g}$(G) \leq 1+r(G)$.
\end{theorem}
...
\begin{proof}
    ...
    Suppose $P=(E,V)$ is a graph of bounded pathwidth $w$. let $\{X_1,X_2,\dots,X_n\}$ be the path decomposition of $P$.
    
    By Theorem \ref{KIERSTEAD1} we give a liner order $L$ on $V$.
    
    Let $L$ be any linear ordering on $V$ such that for any $a,b \in L$, if $a \in X_i, a\notin X_j, b \in X_j$, and $i < j$.
    then  $a < b$.
    
    Worst case each $X_i$ is a clique.
