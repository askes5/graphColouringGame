...
\begin{proof}
    Let $G$ be a graph and $X\subseteq V(G)$ a dominating set such that $|X| = \gamma(G)$. On Alice's first turn she picks any vertex in $X$. Now, suppose Bob has just picked a vertex and $D$ is the current dominating set. Alice's strategy is to pick any unchosen vertex, $v\in X\setminus D$. After $\gamma(G)$ rounds the game must have ended as $X$ is a dominating set and Alice picks a vertex from $X$ in each round. On each round Alice and Bob each add one vertex to $D$, hence the size of $D$ is at most $2\gamma(G)$.
\end{proof}
