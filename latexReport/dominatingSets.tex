...
Let $D$ be the current dominating in the dominating game. In $P_n$ any vertex that Alice chooses will increase the number of dominated vertices ($N(D)$) by at most 3. If Bob picks a vertex in $N(D)$ then he will increase the size of $N(D)$ by at most 1. Hence, after each round $N(D)$ has increased by at most 4 and $D$ has increased by 2. Therefore the game will end after $n/4$ turns with $|D| = 2(n/4) = n/2$. Hence a target score of $n/2$ is needed to ensure that Alice will win.
...
For a better upper bound we introduce the Imagination strategy. The imagination strategy was first ntroducted by Bartnicki, T. and Bre\v{s}ar, B. and Grytczuk, J. and Kov\v{s}e, M. and Miechowicz, Z. and Peterin, I. 2008 \cite{BaBrGrKoMiPe2008} in the context of the colouring game. 
