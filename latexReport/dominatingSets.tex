...
\begin{proof}
    Let $G$ be a connected graph with $n$ vertices. We first need to show that theorem \ref{thm:oreDomUpper} is a tight upper bound.
    
    Consider the path graph with $n/2$ vertices with a single vertex attached to each. That is the graph in figure \ref{}. %todo picture
     A dominating set in $G$ is the set of vertices in the orignal path graph. Hence $\gamma(G)\leq n/2$. Therefore $n/2$ is a tight upper bound for $\gamma(\CC)$. 
...
\end{figure} 
%
\cite{KiWeZa2013} showed conjecture \ref{conj:treedomup} holds for $n$--vertex forests with no isolated vertices where each component is a caterpillar. 
...
Rather than provide a full proof we describe Alice's strategy for $F$ a forest of caterpillars with no isolated vertices. 
    ...
    Let $D$ be the current partial dominating set on Alice's turn. The \textit{residual graph} $F'$ of $F$ is $F$ less any vertices that are totally dominated. That is $F'=F\ba\{v\in V(F): N(v)\subseteq N[D]\}$. The residual graph $F'$ is the subgraph that contains only valid moves. Alice only considers the graph $F'$ when deciding on her next play. Alice considers three cases.
    %
    \begin{enumerate}        
