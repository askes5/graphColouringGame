...
%This can be thought of as Bob being allowed to pick the graph the game is played on. Suppose we have a target score $t$. If Bob is allowed to pick the graph then he will pick a graph $G$ with $\gamma_g(G) > t$. Then if $t\geq \gamma_g(G)$ then it would not matter what graph Bob picks. Alice will always win. 
