...
\begin{definition}[Domination Game Strategy]
    ...
    \end{align*}
    is a strategy if it is defined on any game state and $\varphi_G(D)=D'$ gives a legal move in the dominating game. That is $D'=D\cup \{v\}$ for some $v\in V(G)$ such that $D'$ dominates strictly more vertices than $D$. 
\end{definition}
...
The natural extension of the dominating game is to allow Alice and Bob to select more than one vertex on each turn. We cal this extension as the $(a,b)$--dominating game. In the $(a,b)$--dominating game the only difference is Alice plays $a$ vertices per turn and Bob plays $b$ vertices.

\begin{definition}[Game Domination Number]
    Let $G$ be a graph and $a,b\in\mathbb{N}$ integers. The game domination number $\gamma_g(G;a,b)$ is the smallest target score such that Alice has a winning strategy when playing the $(a,b)$--dominating game.
\end{definition}
