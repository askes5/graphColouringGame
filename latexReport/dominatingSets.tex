...
To inspire the Independent Dominating Game we introduce the cover band \textit{David and the Derivatives}. David and the Derivatives are the headline act at a larger concert. On a normal night there would be no problem filling the concert hall. The problem is, they are under COVID restrictions. This means that all the guests must remain 2m apart. Alice works for the Ministry of health. Her job to to seat as few people as possible while still filling the hall. Bob is David and the Derivatives' manager. His job to maximise the number of people seated in the hall. There are only door sales for concert. So the more people Bob can seat the more profit he and David and the Derivatives make. The guests arrive one by one. Alternating, Alice and Bob direct people to seats. This continuous until no more people can be seated without breaking social distancing. 

The independent domination game is played on a graph $G$ as follows. Starting with Alice. The players take turns picking a vertex $v$ such that $v$ is not incident with any vertex that is already dominated. That is $v\in V(G)\backslash N[D]$, where $D$ is the current dominating set. Alice and Bob continue picking vertices until $D$ forms a dominating set. The score of the game is the size of the dominating set. Alice's goal is to minimise the score, and Bob's goal is to maximise it.

Since an independent dominating set is a maximal independent set and vice versa the independent dominating game is equivalent to the Independent set game. This is called the \textit{competition-independence game}. In this game Alice and Bob take turns constructing a maximal independent set. As before Alice tries to minimise the size of the independent set. The competition-independence number $I_A(G)$ (respectively $I_B(G)$) is the optimal size of a maximal independent set when Alice starts (or Bob respectively). 

The competition-independence game was first introduced in Phillips and Slater 2001 \cite{PhillpsSlater2001}. In \cite{PhillpsSlater2001} the authors show that for a path graph $P_n$,         
\begin{align}
I_A(P_n) &=\lfloor(3n+4)/4\rfloor \label{eq1}\\
I_B(P_n) &=\lfloor(3n+6)/4\rfloor \label{eq2}       
\end{align} 
Further work on bounds competition-independence game was done by Goddard and Henning 2008 \cite{GoddardHenning2018}. In \cite{GoddardHenning2018} the authors show that for any tree $T$ 
\[I_A(T)\leq 4n/7\]
Compared to the domination and total domination game there has been comparatively little work on the competition-independence game. In total there is probably less than a dozen papers on the topic. Recent work has been done in Worawannotai, Ruksasakchai 2020 \cite{WorRuk2020} comparing the competition-independence game to the dominating game. Most of the results in \cite{WorRuk2020} are a variation of the form: For a positive integer $n$, there is some graph $G$ such that
\[\gamma_g(G) - I_A(G) = n\]

For the domination game (and the total domination game) the difference between Alice and Bob starting is at most 1. Interestingly this is not true for the competition-independence game. For an example consider the values for path graphs, equations \ref{eq1} and \ref{eq2}. 

The independence number of a graph $G$ is denoted $\alpha(G)$. It was shown in Borg 2010 \cite{borg2010sharp} that for a graph $G$ with maximal degree $k$ and $n$ vertices 
%
\begin{equation}
\label{equ3}
