...
Rather than provide a full proof we give the main ideas; namely we describe Alice's strategy for $F$ a forest of caterpillars with no isolated vertices. 
    ...
    Let $D$ be the current partial dominating set on Alice's turn. A vertex is \textit{totally dominated} if all it's neighbours are in the dominating set. The \textit{residual graph} $F'$ of $F$ is $F$ less any vertices that are totally dominated. That is $F'=F\ba\{v\in V(F): N(v)\subseteq N[D]\}$. The residual graph $F'$ is the subgraph that contains only valid moves. Alice only considers the graph $F'$ when deciding on her next play. Alice considers three cases.
    %
    \begin{enumerate}        
    ...
    \end{enumerate}
The remainder of the proof done using induction on the number vertices in the graph. We would use the fact that if $F$ is the union of two forests, that is $F=F_1\cup F_2$, then $\gamma_g(F)\leq\gamma_g(F_1) +\gamma_g(F_2) +1$. 
...
\begin{theorem}[Bujt\'{a}s, Csilla 2015 \cite{Bujt2015}]\label{thm:domtreedist4}
    ...
    $\gamma_g(T)\leq 3n/5$.
\end{theorem}
Again, rather than provide a full proof we give the main ideas; namely we describe Alice's strategy. As part of this strategy Alice assigns a value to each vertex $v$ as follows, 
\begin{itemize}
