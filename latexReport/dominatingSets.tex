...
Theorem \ref{thm:gamedomLow} does not say that for all connected graphs with domination number less than $n/2$, Alice will always lose. Rather a target score greater than $n/2$ is needed to ensure Alice will always win on any arbitrary connected graph. This can be seen by considering the path graph $P_n$. That is the graph with $n$ vertices connected in a single line. %todo insert picture 
In $P_n$ any vertex that Alice chooses will increase the size of dominated vertices, that is the size of the set $N[D]$, by at most 3. If Bob picks a vertex in $N(D)$ then he will increase the size of $N[D]$ by at most 1. Hence, after each round $N[D]$ has increase by at most 4 and $D$ has increase by 2. Thus when $D$ forms a dominating set $|D|\leq n/2$.
