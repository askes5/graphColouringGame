...
\begin{theorem}[Bujt\'{a}s, Csilla 2015 \cite{Bujt2015}]\label{thm:domtreedist4}
    ...
    $\gamma_g(T)\leq 3n/5$.
\end{theorem}
In Alice's strategy for theorem \ref{thm:domtreedist4} there are 4 stages. The stages define what vertices Alice can play. The game starts in stage 1. If on Alice's turn there is no suitable vertex at that stage then she moves to the next stage. To choose vertices Alice assigns a value to each vertex $v$ as follows: 
\begin{itemize}
    \item $v$ has value 3 if it is not dominated.
    \item $v$ has value 2 is it is partially dominated.
    \item $v$ has value 0 if it is totally dominated.
\end{itemize}
The gain of a play is the difference between the sum of all the values in the tree before and after each play.
