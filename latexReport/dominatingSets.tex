...
Consider some graph $G=(V,E)$, the vertex set $V$ is a dominating set. Therefore when $D$ forms a dominating set $|D|\leq |V(G)|$. Thus a game with target score $|V(G)|$ guarantees Alice will win. Hence $|V(G)|$ is an upper bound for the game domination number. That is, $\gamma_g(G) \leq |V(G)|$.
...
\begin{theorem}[Bre\v{s}ar and Klav\v{z}ar and Rall 2010 \cite{BrKlRa2010}]\label{thm:gamedomup}
    ...
    \[\gamma_g(G)<2\gamma(G)\]
\end{theorem}
In \cite{BrKlRa2010} the authors do not give a formal proof, they give a brief sketch. This is something we aim to remedy.. 
\begin{proof}
...
A \textit{caterpillar graph} is a tree in which all the vertices lie on a path or have distance at most one from a central path. Figure \ref{fig:catgraphs} provides some examples.
%
\begin{figure} [h]
    \centering
...
Further work has been done to improve the class of trees that the conjecture \ref{conj:treedomup} holds for. Bujt\'{a}s, Csilla 2015 \cite{Bujt2015} showed that conjecture \ref{conj:treedomup} holds for forests that have no isolated vertices and in which no two leaves have distance 4.
%
...
\end{theorem}
Again, rather than provide a full proof we describe Alice's strategy. As part of this strategy Alice assigns a value to each vertex $v\in V(F)$ as follows, 
\begin{itemize}
...
This fact is not true for the competition-independence game. As an example consider the star graph $S_7$ in figure \ref{fig:starS7}. 
%
\begin{figure}[h]
    \centering
    \begin{tikzpicture}  
...
\begin{definition}[Independence number]
    The \textit{independence number} of a graph $G$, denoted $\alpha(G)$, is the size of a maximum independent set in $G$.
\end{definition}
