...
\begin{theorem}
    Let $\mathcal{P}$ be family of graphs with bounded pathwidth $w$. If we have $p\geq2$ players then \[pw+\left\lceil \frac{p-1}{2}\right\rceil\leq\chi_g(\mathcal{P};p)\]
\end{theorem}
%todo lower bound of pw+1.
% thus we need at least w extra colours for each extra player?? equilevlant to incrasing the pathwitdth by 1??
...
\begin{proof}
    It suffices to show that there exists a graph in $\mathcal{P}$ for which bob has a winning strategy with $m=pw+\left\lceil\frac{p-1}{2}\right\rceil-1$ colours. Let the available colours be $C=\{c_1,c_2,\dots,c_m\}$.
    
    We define the graph $G$ as follows. First start with a $w$-clique, $K_w$. Second take $n=p^2w$ vertices, $v_1,\dots,v_n$ and connect each $v_i$ to each vertex in $K_w$, forming a $w+1$-clique. Now copy this graph,  and connect the copies at any vertex not in $K_w$. We now have the graph, $G$, in figure \ref{fig:pwlowerbound1}. Note: $v_1=t_1$. 
    
    Note that $G$ has the path decomposition $\{K_w\cup\{v_n\}, K_w\cup\{v_{n-1}\}, \dots, K_w\cup\{v_1\}, K_w'\cup\{v_1\}, K_w'\cup\{t_2\}, \dots, K_w'\cup\{t_n\}\}$, and therefore has path width $w$.
\begin{figure}[H]
    \centering
...
\node[normal, label=below:$s_{w-1}$] (v9) at (1.5,0) {};
\node[normal, label=below:$s_{w}$] (v10) at (3,0) {};
\draw  (v7) edge (v1);
\draw  (v1) edge (v8);
\draw  (v1) edge (v9);
\draw  (v1) edge (v10);
...
\node[normal, label=below:$u_{w-1}$] (v19) at (1.5,10) {};
\node[normal, label=below:$u_{w}$] (v110) at (3,10) {};
\draw  (v17) edge (v11);
\draw  (v11) edge (v18);
\draw  (v11) edge (v19);
\draw  (v11) edge (v110);
...
Therefore on Bobs first turn he colours $v_1$ $c_1$ and as many of $v_2,t_2,v_3,t_3,\dots$ in order that he can on his turn.
 By symmetry of $G$ was can assume Alice colours one of $t_2,\dots,t_n,s_1,\dots,s_w$. 
...
\node[normal, label=below:$u_{w-1}$] (v9) at (1.5,0) {};
\node[normal, label=below:$u_{w}$] (v10) at (3,0) {};
\draw  (v7) edge (v1);
\draw  (v1) edge (v8);
\draw  (v1) edge (v9);
\draw  (v1) edge (v10);
...
Bob's strategy is to always colours $p-1$ uncoloured vertices not in $K_w$ with colours not used in $H$. We keep playing until either $K_w$ is fully coloured bar one, or Bob runs out of new colours. We consider the cases separately 

First, suppose $K_w$ is fully coloured bar one. let the uncoloured vertex be $u_w$. On the first turn Bob coloured $p-1$ vertices, but at most half of these are in $H$ and so Bob coloured $\left\lceil \frac{p-1}{2}\right\rceil$ vertices in $H$. On the second turn Alice coloured no vertex in $H$ and Bob $p-1$ vertices. As each coloured vertex in $K_w$ must have been coloured by Alice there have been at least $w-1$ turns after the second turn and each turn Bob coloured $p-1$ vertices. Finally when each vertex in $K_w$ was coloured the vertex must have coloured differently than the ones before it. Therefore the total number of unique coloured neighbours of $u_w$ is 
\[\left\lceil \frac{p-1}{2}\right\rceil+(p-1) +(p-1)(w-1)+(w-1)=pw+\left\lceil \frac{p-1}{2}\right\rceil-1\]
Therefore $u_w$ cannot be coloured, and Bob wins.

Every turn Bob uses $p-1$ unique colours. Therefore it will take at least $(pw-1)\div(p-1)$ turns to exhaust all the colours. Each turn $p$ vertices are coloured and here are $n=p^2w$ vertices for Bob to colour. $p^2w > p(pw-1)\div(p-1)$ . Therefore Bob cannot run out of vertices to colour before he runs out of colours.

Second, suppose Bob has run out of colours then every uncoloured vertex in $K_w$ is connected to $pw-1$ many unique coloured neighbours and thus cannot be coloured, therefore Bob has won.
