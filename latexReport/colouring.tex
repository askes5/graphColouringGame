...
For us the motivation for the marking game is to find strategies for the colouring game. But thi is not the only reason you might be interested in the marking game. Think back to the dinner party problem. Originally, when hosting parties Alice would feed everyone buffet style. But she caught on to Bob's strategy. So, to cut down on costs she decides everyone gets a set plate. Now there is no problem about everyone getting feed. However, there is a new problem. The guests get upset if too many of their immediate neighbours are feed before them. Define a graph $G$ with guests as vertices and and edge between a pair of guests if they care about each other being feed. Alice wants to make the party go as smoothly as possible. That is she wants a ordering $L$ of guests such that $\delta^+_L(G)$ is minimised. 
To help hand out the plates Alice enlists the help of her friend Bob. Alice and Bob will tkae turns passing out plates. In effect, they will play the marking game on $G$. There is a snag. Alice is sleeping with Bob's girlfriend. Alice doesn't know that Bob knows. As revenge Bob wants to ruin Alice's party. To do this he will attempt to pass out plates that maximises $\delta^+_L(G)$. For any guest what the is least number of neighbours can Alice guarantee are feed before the guest? That is, what is the game colouring number?
...
Faigle, Kern, Kierstead, and Trotter 1993 \cite{faKeKiTr1993} introduced a winning strategy with four colours for Alice on the class of forests. We present the strategy here to give some inspiration for the activation strategy. While this example misses some nuance of the full activation strategy, the basic premise is the same.  
