...
We consider the colouring game played between two players, Alice and Bob. Let $G$ be any graph, and $C$ a set of colours.Beginning with Alice, ALice and Bob take alternating turns. On their turn they choose an uncoloured vertex, $v$, and assigns $v$ a colour from $C$ such that no two adjacent vertices in $G$ have the same colour. This continues until one of two win conditions are meet. First, Alice wins if all the vertices are coloured. Second, Bob wins if there is a vertex that cannot be coloured with the available colours.
...
We can also extend the colouring game to have $p$ players. The win conditions and rules are the same as the standard game but for every turn Alice has Bob has $p-1$ turns. This can be seen as there being $p$ different players Alice, Bob, Charlie, etc. where a win for Bob is considered a win for all players apart from Alice.
\begin{definition}
    let $G$ be a graph, and $p\in\mathbb{N}$. 
     Then, $\chi_g(G;p)$ is the smallest number of colours needed to guarantee that Alice has a winning strategy on $G$, when playing the colouring game with $p$ players. 
\end{definition}
Note that $\chi_g(G) = \chi_g(G;2)$.

\begin{definition}
    Let $G$ be a graph. Then, $\col(G)$ is the score on $G$, when playing the marking game.
\end{definition}
...
The first results we will look at are results the stem directly from strategies for Alice and Bob.

\begin{theorem}
    Let $\TT$ be the family of trees. If we have $p \geq 2$ players then,
    \[\chi_g(\TT;p) \geq p + 2 \]
\end{theorem}
...
%        (v12) -- (v4) node[midway,yshift=-3em]{$p$ vertices};
        \draw[decorate, decoration={brace, amplitude=10pt,mirror,raise=2ex}, thick] ($(v12)+(-.25,0)$)--($(v4)+(.25,0)$) node[midway,yshift=-3em]{$p$ vertices};
        \end{tikzpicture}
        \caption{}
    ...
    Let the set of available colours be $\{c_1,c_2,\ldots,c_p,c_{p+1}\}$.
...
\end{figure}
    ...
    Therefore we have a winning strategy on $G$ in $\TT$ for Bob with $p+1$ colours. 
\end{proof}
...
%conjecture that tree is (p+1)w+1
 ...
 Note that having $p$ players is equivalent to having Bob have $p-1$ turns for every turn Alice has.
\begin{proof}
    It suffices to show that there exists a graph in $\PP$ for which bob has a winning strategy with $m=pk+\left\lceil\frac{p-1}{2}\right\rceil-1$ colours. Let the available colours be $C=\{c_1,c_2,\dots,c_m\}$.
    
    We define the graph $G$ as follows. First start with a $k$-clique, $K_k$. Second take $n=p^2k$ vertices, $v_1,\dots,v_n$ and connect each $v_i$ to each vertex in $K_k$, forming a $k+1$-clique. Now copy this graph,  and connect the copies at any vertex not in $K_k$. We now have the graph, $G$, as in figure \ref{fig:pwlowerbound1}. Note: $v_1=t_1$. 
...
We define the marking game as follows. Let $G$ be a graph, and $n$ a target score. Starting with Alice, Alice and Bob take turns to choose an unchosen vertex, $v$ in $G$. The order in which the vertices were chosen forms a linear order, $L$. The score of the game is $\max_{v\in V(G)}\{d^+_L(v)\}$. Alice wins is the score is less than $n$ and Bob wins otherwise.

\begin{definition}[Colouring number]
    Suppose $G$ is a graph. The colouring number $\col(G)$ is the minimum target score in the marking game such that Alice has a winning strategy. 
\end{definition}

The marking game is commonly used for finding upper bounds for the game chromatic number, and was formally introduced in \cite{Zhu1999}, further examples can be found in  \cite{KIERSTEAD2000,Zhu2008}.
...
Let $\mathcal{F}$ be the family of forests, $\mathcal{I}_k$ the family of interval graphs with clique number $k$, $\mathcal{P}$ the family of planar graphs, $\mathcal{Q}$ the family of outer planar graphs, and $\mathcal{K}_k$ the family of partial k trees. %note K_k is exactly the graphs of bounded tree width.

The colouring number is known for a variety of different c

\subsection{Summery of activation strategy}

The activation strategy was introduced by H. A. Kirstead in \cite{KIERSTEAD2000}.

Consider a graph $G(v,E)$ with a linear ordering $L$ on $V$. $U$ is the set of unmarked vertices, and $A$ the active ones.

Alice starts by marking the least $v$ in $L$. If $u$ is the last marked vertex by Bob, Alice starts at $u$ activates it and moves to the least unmarked neighbour of $u$, say $w$, in $L$. If $w$ is active Alice marks $w$, if not Alice repeats this on $w$ until she finds an active vertex.
...
The following result is not new, see \cite[Wu, Zhu 2008]{WuZhu2008}, but the proof proved here is based on the activation strategy.
...
\begin{theorem}[\cite[Theorem 1]{Zhu2008}]
    Let $G$ be a planar graph, then \[\chi_g(G)\leq\col(G)\leq17\]
\end{theorem}
