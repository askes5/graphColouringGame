...
\begin{theorem}
    Let $\mathcal{P}$ be family of graphs with bounded pathwidth $w$. If we have $p\geq2$ players then \[2w+1\geq\chi_g(\mathcal{P};p)\]
\end{theorem}
...
\begin{proof}
    It suffices to show that there exists a graph in $\mathcal{P}$ for which bob has a winning strategy with $2w$.
    
    We define the graph $G$ as follows. First start with a $w$-clique, $K_w$. Second take $n=pw$ vertices, $v_1,\dots,v_n$ and connect each $v_i$ to each vertex in $K_w$. Now copy this graph,  and connect by copies at any vertex not in $K_w$. We now have the graph, $G$, in figure \ref{fig:pwlowerbound1}.
    
    $G$ has the path decomposition $\{K_w\cup\{v_n\}, K_w\cup\{v_{n-1}\}, \dots, K_w\cup\{v_1\}, K_w'\cup\{v_1\}, K_w'\cup\{t_2\}, \dots, K_w'\cup\{t_n\}\}$, and therefore has path width $w$.
\begin{figure}[H]
    \centering
