...
\begin{definition}[Vertex separation number]
...
\begin{theorem}[Kinnersley \cite{Kinnersley1992}]\label{kinnersley}
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        \Require {$\left( \mathcal{X}=\{X_i:i\in I\}, T \right) $ is the tree decomposition of a graph $G$. $r$ is the root of $T$}
        \Ensure {$L$ is a linear order on $V(G)$}
        
        \Function{LOinTree}{$(\mathcal{X},T)$, $r$}        
            \State $L \gets \emptyset$ \Comment our linear order for $V(G)$
...
\begin{proof}
    ...
    As $G$ is a k-tree $G$ is maximal. That is each $X_i$ is a clique.
    
    Let $v$ be a vertex in $G$ It suffices to show $r(v,L,G) \leq 3k + 1$. Recall $r(v,L,G) = d^+(v) + m(v,L,G)$.
...
%        Consider $T$ as a rooted tree with root $r$.         
%        Define the depth of $u \in V(G)$, depth$(u)$, as the length of the shortest path from $X_i$ to $r$ in $T$, where $u\in X_i$. Note that for any $u\in V(G)$, each vertex in $V^+(u)$ has depth less than or equal to depth$(u)$ and each vertex in $V^-(u)$ has depth greater than or equal to depth$(u)$.         
        %M from X to V^+(v)
%        . BY defination of tree decompositon For any edge $e =(a,b)$ in $M$ there is some $X_i \in T$ such that $a\in X_i$ and $b\in X_i$.         
%        Let $X_k$ be the closest vertex in $T$ to $r$ that contains $v$. The path from $X_i$ to $X_j$ in $T$ contains $X_k$ as $v\in X_k$ and $v < b$. There are less than or equal to $k$ edges between the vertices in $T$ therefore $|M| \leq k$        
        %N from Y to ^+[v] 
\end{proof}
...
\begin{proof}
    ...
    Adding edges to a graph doesn't decease its game chromatic number. Thus add edges to $G$ until $G$ is maximal. By defination $G$ is now a k-tree. Therefore by theorem \ref{ktree} \[\chi_g(G) \leq 3w + 2\]
\end{proof}
