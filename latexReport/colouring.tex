...
\begin{proof}[Proof of theorem \ref{thm:colTreeleq4} \cite{KIERSTEAD2000}]
    ...
    It remains to show that if there exists matchings $M:X\to V^+(v)$ and $N:Y\to V^+[v]$ then $|Z| = |X|+|Y| \leq 2$.
...
The next result we show is theorem \ref{thm:pathwidth}. It is not a new result, a proof apair in \cite{faKeKiTr1993}. 
...
\begin{figure}[h]
    ...
    \node [normal, label=left:$a$] (v1) at (0,1) {};
    \node [normal, label=above:$b$] (v2) at (2,2) {};
    \node [normal, label=above:$c$] (v5) at (3.5,2) {};
    \node [normal, label=right:$d$] (v6) at (4,0) {};
    \node [normal, label=left:$e$] (v3) at (-1,2) {};
    \node [normal, label=left:$f$] (v4) at (-0.5,0) {};
...
The interval width of a graph is the size of the maximal clique in 
