...
\begin{theorem}[Kinnersley \cite{Kinnersley1992}]\label{thm_kinnersley}
...
\begin{proof}
    Suppose $P=(E,V)$ is a graph of bounded pathwidth $k$ and let $\mathscr{X} = \{X_1,X_2,\dots,X_n\}$ be a path decomposition of $P$ with width $k$.
...
\begin{proof}
    ...
    $G$ is a subgraph of a k-tree. Therefore by theorem \ref{Thm_ktree} \[\chi_g(G) \leq 3w + 2\]
\end{proof}
