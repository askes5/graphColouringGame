...
\begin{proof}
    ...
    As $\chi_g(P) \leq$ col\textsubscript{g}$(P)$ and col\textsubscript{g}$(G)\leq 1+r(G)$. By theorem \ref{KIERSTEAD1} it suffices to show $r(u,L,G) \leq 2w + 1$.
        
    Let $L$ be a linear ordering on $V$ generated by algorithm \ref*{LinOrderPW_algo}. 
    Let $v$ be any vertex in $V$. 
    
    By the definition of path width  all $X_i \in \mathcal{X}$, $ \left| X_i \right| \leq w + 1 $. And $d^+(v) \leq w$ by theorem \ref{kinnersley}.

    Let $Z\subset N^-[v]$.
    
    By lemma \ref{neighboursAfter} $N^-[v] \subseteq X_i$ for some $X_i \in \mathcal{X}$ . Therefore $\left| Z\right| \leq \left| N^-(v)\right| \leq |X_i| \leq w+1$. Therefore by definition \ref{defnRank} $m(u,L,P) \leq w+1$ and $r(u,L,G) = d^+_{G_L}(u) + m(u,L,G) \leq 2w +1$.
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        \Require {$\left( \mathcal{X}=\{X_i:i\in I\}, T \right) $ is the tree decomposition of a graph $G$}
        \Ensure {$L$ is a linear order on $V(G)$}
        
        \State $L \gets \emptyset$ \Comment our linear order for $V(G)$
        \State let $R$ be an vertex in$T$        
...
\begin{lemma}
    Let $G=(E,V)$ be a graph of bounded treewidth $w$ and $(\{X_1,X_2,\dots,X_n\},T)$ be a tree decomposition of $G$ with width $w$.
    
    Suppose $L$ is a linear ordering on $V$ generated by algorithm \ref{LinOrderTW_algo} and $v$ is any vertex in $G$. Then $|N^+(v)| \leq w$. 
\end{lemma}

%todo prove the above lemma

\begin{theorem}
        Let $G$ be a graph of bounded pathwidth $w$. Then, 
        \[\chi_g(P) \leq 3w + 2\]
\end{theorem}
...
\begin{proof}
        ...
        Tt suffices to show col\textsubscript{g}$(P) \leq 3w + 2$. 
\end{proof}
