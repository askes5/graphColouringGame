...
\begin{definition}[Asymmetric Activation Strategy, \cite{yangZhu2008}] 
    Let $G$ be a graph, $U$ denote the set of unmarked vertices, and for all $v\in V(G)$ let $t_v=a$, track the number of activations of $v$. If $v$ has been activated $a$ times then $v$ will have $t_v=0$. Let $L$ be a linear order of on $V(G)$, and let $G_L$ be the direction induced  by $L$ on $G$. %Let $L_v$ be a linear order on $N^+_{G_L}(v)$. 
