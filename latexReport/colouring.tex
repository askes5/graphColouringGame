...
\begin{definition}[Path decomposition]
    Let $G$ be a graph. A path decomposition is list of subsets of $V(G)$, $P={P_1,P_2,\dots,P_n}$ that have the following properties.    
    \begin{enumerate}[(i)]
        \item For all edges $uv$ in $G$ there exits an $i$ such that such $u,v\in P_i$
        \item If there exits an $i\leq j$ and a vertex $v$ such that $v\in P_i$ and $v\in P_j$ then for all $i<l<j$ $v\in P_l$
    \end{enumerate}
    The width of a path decomposition is the size if the largest set in $P$.
\end{definition}
