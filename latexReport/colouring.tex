...
We will now temporally set aside trees and look at graphs of bounded pathwidth. The pathwidth of a graph can be considered a measure of how close to path the graph is. 
\begin{figure} [h]
    \centering
    ...
    \end{minipage}
\end{figure} 
For example, the graphs of path width 1 are caterpillars and unions of caterpillars, such graphs are almost paths (see figure \ref{fig:pathwidth1}). Figure \ref{fig:pathwidth2} is a graph of pathwidth 3 and is noticeably less treelike. 
...
\begin{proof}[Proof of Theorem \ref{thm:colTreeleq4} \cite{KIERSTEAD2000}]
    ...
    Assume there exists such matchings $M$ and $N$. In a tree every the deletion of any non--leaf disconnects the graph. So in $T\ba v$, $N^-(v)$ and $V^+(v)$ are disjoint. Thus there exists no edges from $N^-(v)$ to $V^+(v)$. Therefore $M$ can contain at most the single edge $(v,u)$ for some $u\in N^+(v)$. Thus the only vertex $X$ can contain is $v$.    
