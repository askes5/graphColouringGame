...
Introduced by by H. A. Kirstead in \cite{KIERSTEAD2000} the activation strategy is strategy for Alice to use to play the marking game. IT is built on the work in \cite{faKeKiTr1993}, where the authors introduced a winning strategy for Alice in the class of forests. They showed that $\chi_g(\FF)\leq4$. 
...
The game colouring number for these classes of graphs are known. %todo either remove this or add refferences
...
We will now provide some basic results that demonstrate typical usage of the activation strategy. We bound the families of bounded pathwidth and of bounded tree width.
...
The bounds found with the limited Harmonious Strategy, while good, are not as tight as the Harmonious Strategy. This provided the motivation for Yang and Zhu to improve the strategy. In \cite{yangZhu2008} they introduce a new stragety based on their popular activation stragety. They summarise it as follows:
